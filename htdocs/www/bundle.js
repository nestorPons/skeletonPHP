...
$_GET[_decode(arguments[1])]=_decode(arguments[2])});const app={ver:'2.2',timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone,getView(view='index',data={},load=!1,container='main',callback){let d={'view':view,'data':data}
...
typeof callback=='function'&&callback(html)},'html')},post(data,callback,error=!0){if(typeof navbar!='undefined')
...
if(format)
return this.format(date,format);else return date},sql(param=this.date){return this.format(param,'sql')},short(param=this.date){return this.format(param,'short')},hour(param=this.date){return this.format(param,'hour')},datetime(param=this.date){return this.format(param,'datetime')}}
var echo=function(){for(let i in arguments)
console.log(arguments[i]);};var exist=function(arg=undefined){return arg!=undefined&&arg!=null}
var remove=function(arr=[]){do{let b=arr[0]
if(b)
b.remove()}while(arr.length>0)}
